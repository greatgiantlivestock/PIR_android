...
public class ProspectPlanVisit extends ActionBarActivity implements NavigationDrawerCallbacks {
	private Context act;
	...
	private static final String LOG_TAG = ProspectPlanVisit.class.getSimpleName();
...
//	private ViewPager viewPager;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_prospect_plan_visit);
	...
	private void ChooseCustomerDialog() {
		...
		final EditText searchCustomer = (EditText) chooseCustomerDialog
				.findViewById(R.id.activity_product_edittext_search);
		final EditText desa_ = (EditText) chooseCustomerDialog
				.findViewById(R.id.desa_input);
		keteranganPetani = (EditText) chooseCustomerDialog
				.findViewById(R.id.activity_product_edittext_keterangan);
		Button addProspect = (Button) chooseCustomerDialog
				.findViewById(R.id.add_petani);
		...
		addProspect.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
...
//					});
		...
		protected String doInBackground(String... params) {
			...
			String url_add_req_load = AppVar.CONFIG_APP_URL_PUBLIC
					+ AppVar.CONFIG_APP_URL_UPLOAD_RENCANA_DETAIL_URGENT_PROSPECT;
			for (DetailReqLoadNew detailReqLoad : detailReqLoadList) {
				response_data = uploadReqLoadDetail(
						...
						id_karyawan,detailReqLoad.getKeterangan(),detailReqLoad.getNama_product(),detailReqLoad.getAlamat()
				);
			}
			return null;
		}
...
//		public ListViewChooseAdapter(Activity mainActivity,int layoutResourceId, EditText jumlahProduct, EditText batch,ArrayList<Product> data, Dialog chooseProductDialog) {
		...
		public View getView(final int position, View convertView,
			...
			if (row == null) {
				...
				holder.mButtonAddItem = (Button) row.findViewById(R.id.sales_order_dialog_btn_add_item);
...
//			TextView list_kodeProduct;
	...
	private String uploadReqLoadDetail(final String url,
		...
		try {
			...
			entity.addPart("desa", new StringBody(desa));
			httppost.setEntity(entity);
			// Making server call
			HttpResponse response = httpclient.execute(httppost);
			HttpEntity r_entity = response.getEntity();
